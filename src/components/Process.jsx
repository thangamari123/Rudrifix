...
export default function Process() {
  ...
  return (
    <section id="process" className="relative overflow-hidden py-16 sm:py-20" style={{ background: 'linear-gradient(135deg, #FAFBFF 0%, #F6F3FF 45%, #EEF4FF 100%)' }}>
      {/* Background Decorative Elements */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-purple-100/30 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10" ref={ref}>
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-1/2 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-indigo-100 to-transparent -translate-y-1/2" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
            {steps.map((step, i) => (
              ...
              >
                {/* Node Container */}
                <div className="h-full bg-white/80 backdrop-blur-xl border border-white/60 rounded-[2.5rem] p-8 sm:p-10 shadow-xl shadow-slate-200/20 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl flex flex-col">
                  {/* Step ID */}
                  ...
                  {i < steps.length - 1 && (
                    <div className="hidden lg:block absolute -right-4 top-1/2 -translate-y-1/2 z-20">
                      <div className="w-8 h-8 rounded-full bg-white shadow-lg border border-slate-100 flex items-center justify-center text-slate-300">
                        <ArrowRight size={16} />
                      </div>
                    </div>
                  )}
